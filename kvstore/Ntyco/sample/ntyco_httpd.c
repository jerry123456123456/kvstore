/*
这段代码是一个 基于多进程与协程混合架构的 HTTP 服务器，结合了操作系统多进程（CPU 亲和性绑定）和用户态协程（轻量级并发），主要功能是处理 HTTP 请求并返回固定的 HTML 页面。
启动服务器：./ntyco_httpd
curl测试：curl http://localhost:9001
http://localhost:9001，页面显示 "Welcome to nginx!"（实际由代码中的 HTML_PAGE 定义）
*/
...
int main(int argc, char *argv[]) {
	...
	int ret = bind(fd, (struct sockaddr*)&local, sizeof(struct sockaddr_in));
