/*
该程序主要实现了对 MySQL 数据库中 TBL_USER 表的增删查操作，同时还涉及到将图片数据存储到数据库以及从数据库中读取图片数据并保存为文件的功能。具体功能包括：
向 TBL_USER 表中插入普通数据。
查询 TBL_USER 表中的数据并打印结果。
调用存储过程删除 TBL_USER 表中的数据。
读取本地图片文件并将其存储到数据库中。
从数据库中读取图片数据并保存为本地文件。
*/
...
#define KING_DB_SERVER_IP		"192.168.186.138"
#define KING_DB_SERVER_PORT		3306

#define KING_DB_USERNAME		"jerry"
#define KING_DB_PASSWORD		"123456"

#define KING_DB_DEFAULTDB		"JERRY_DB"


#define SQL_INSERT_TBL_USER		"INSERT TBL_USER(U_NAME, U_GENDER) VALUES('jerry', 'man');"
#define SQL_SELECT_TBL_USER		"SELECT * FROM TBL_USER;"

#define SQL_DELETE_TBL_USER		"CALL PROC_DELETE_USER('jerry')"
#define SQL_INSERT_IMG_USER		"INSERT TBL_USER(U_NAME, U_GENDER, U_IMG) VALUES('jerry', 'man', ?);"

#define SQL_SELECT_IMG_USER		"SELECT U_IMG FROM TBL_USER WHERE U_NAME='jerry';"
